...
plot_shot_AI <- function(data, type = "", teamColor = "#B22222", text_color = "", bins = 30, highlight_goals = "", average_location = "", matchFixture = "", mapName = "", mapType = "", matchDate = "", dataSource = "", logo = "", wrap = "") {
  ...
  glue("<span style='font-family:\"Arial Bold\";color:{teamColor};font-size:16px;'>{player_name}</span><span style='font-family:\"Arial\";color:gray12;font-size:16px;'> Shot Map (</span><span style='font-family:\"Arial Bold\";color:{teamColor};font-size:16px;'>Goal,</span><span style='font-family:\"Arial\";color:{teamColor};font-size:16px;'> SoT</span><span style='font-family:\"Arial\";color:gray12;font-size:16px;'> and </span><span style='font-family:\"Arial Bold\";color:#95A5A6;font-size:16px;'>No Goal</span><span style='color:gray12;font-size:16px;'>)</span>"))
    ...
    mapping = aes(x = X, y = Y, image = "~/Dropbox/AI LFC data/Icons/ball3.png"), 
...
#set Away color
Color_Away <- paste("ivory")

#set Away text
Color_Away_Text <- paste("firebrick4")

#set away color scale
ColorScale_Away <- paste("RColorBrewer::Reds")

#set away color scale con
ColorScale_AwayC <- paste("grDevices::Reds 2")
...
#set game week
GameWeek <- paste("MD", MatchDay)

#set viz location for camcorder adn blue sky images
Vizlocation <- paste0("~/Dropbox/AI LFC data/R files/AI UP/Scripts/Viz Pack/Season 2526/EPL/", GameWeek)
...
Tertiary_title <- paste0(
              glue("<span style='font-family:\"Arial\";color:gray12;font-size:16px;'>Match  </span><span style='font-family:\"Arial Bold\";color:{Color_Home};font-size:16px;'>Possession</span><span style='color:grey12;font-size:16px;'> to </span><span style='font-family:\"Arial Bold\";color:{Color_Away_Text};font-size:16px;'>Field Tilt</span><span style='color:gray12;font-size:16px;'> conversion</span>"))
...
```{r}
#| output: false
#| label: load in data frames
#| message: false
#| warning: false

fileName <- paste0("~/Dropbox/AI LFC data/Data Sets/WhoScored/Season 2425/EPL Games/Processed Games/", Match_Fixture, ".xlsx")
plottingData <- read_xlsx(fileName)

PlayerfileName <- paste0("~/Dropbox/AI lfc data/Data Sets/WhoScored/Season 2425/EPL Players/Processed Games/", Match_Fixture, "- PlayerData.xlsx")
...
ggplot() +
  ...
  geom_ribbon(data = rib_away,
              aes(x = expandedMinute, ymin = ymin, ymax = ymax),
              fill = Color_Away_Text, alpha = 0.30, color = NA) +
  ...
  geom_line(data = seg, aes(x = expandedMinute, y = away_s),
            linewidth = 1.2, color = Color_Away_Text) +
  
  scale_fill_manual(
    values = c("home" = Color_Home, "away" = Color_Away_Text),
    name   = "Tertiary",
  ...
  annotate("label",
           ...
           size = 5, color = Color_Home_Text, fontface = "bold", fill = Color_Home) +
  annotate("label",
           x = max(pred$expandedMinute) / 2, y = 0.05,
           label = paste(Match_Away, " Defensive Goal Line"), vjust = 1, hjust = 0.5,
           size = 5, color = Color_Away, fontface = "bold", fill = Color_Away_Text)
...
PassConnections <- PassConnections %>%
  filter(Attempted_Passes == 1) %>% 
  select(Passer = playerId, Receiver = receiver, Passer_Team = TeamName, Receiver_Team = receiver_team, Successful_Passes, BCs, Key_Passes, Prog_Passes, EPV = EPVvalue, x, y, Endx, Endy, PK_Box_Passes, Final_Third_Passes, Cross_Passes, Long_Passes) 

#calculate pass success
PassConnections <- PassConnections %>%
  group_by(Passer, Receiver, Passer_Team, Receiver_Team) %>% 
  summarise(Passes = n(), Successful_Passes = sum(Successful_Passes), Long_Passes = sum(Long_Passes), Final_Third_Passes = sum(Final_Third_Passes), PK_Box_Passes = sum(PK_Box_Passes), Crosses = sum(Cross_Passes), Key_Passes = sum(Key_Passes), BCs = sum(BCs), Prog_Passes = sum(Prog_Passes), EPV = sum(EPV)) %>% 
  na.omit() %>%
...
PassConnections <- PassConnections %>%
  ...
  filter(Passes >= 5) %>%
  summarise(Passes = sum(Passes), Successful_Passes = sum(Successful_Passes), Pass_Success = mean(Pass_Suc), Long_Passes = sum(Long_Passes), 'Final Third Passes' = sum(Final_Third_Passes), 'PK Box Passes' = sum(PK_Box_Passes), Crosses = sum(Crosses), Key_Passes = sum(Key_Passes), BCs = sum(BCs), Prog_Passes = sum(Prog_Passes), EPV = sum(EPV), EPV_P_Pass)

#prep for main tables
PassConnectiondata <- PassConnections %>%
  select(Passer_Team, Passer, Receiver, Receiver_Team, 'Attempted Passes' = Passes, 'Successful Passes' = Successful_Passes, 'Pass Success %' = Pass_Success, 'Long Passes' = Long_Passes, 'Final Third Passes', 'PK Box Passes', Crosses,  'Key Passes' = Key_Passes, 'Prog Passes' = Prog_Passes, EPV, 'EPV / Pass' = EPV_P_Pass)
...
HomepassPlot1subAttpost <- plot_passnet_AI(data = plottingData,  
                                    team_name = Match_Home, scale_stat = "EPV", 
                                    scale_color = Color_Home,  text_color = Color_Home_Text, subtitle = paste0(Match_Fixture, " Post 1st Subs"),
                                    plot_start = firstSubHome, plot_finish = 100, pitch = "Half", pressmap = "Yes")
...
AwaypassPlot1SubAttpost <- plot_passnet_AI(data = plottingData,
                                      team_name = Match_Away, scale_stat = "EPV", 
                                      scale_color = Color_Away_Text,  text_color = Color_Away, subtitle = paste0(Match_Fixture, " Post 1st Subs"),
                                      plot_start = firstSubAway, plot_finish = 100, pitch = "Half", pressmap = "Yes")
...
AwaypassPlotFTAttW <- plot_passnet_AI(data = plottingData,  
                                     team_name = Match_Away, scale_stat = "EPV", 
                                     scale_color = Color_Away_Text,  text_color = Color_Away, subtitle = paste0(Match_Fixture, " By Time Period"),
                                     plot_start = 00, plot_finish = 100, pitch = "Half", pressmap = "Yes", wrap = "Yes", passes = 1)
AwaypassPlotFTAttW
```
