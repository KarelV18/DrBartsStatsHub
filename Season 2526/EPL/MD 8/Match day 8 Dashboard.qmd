...
facet_order <- touch_area_data %>%
  mutate(role = factor(role, levels = role_levels)) %>%
  distinct(facet_val = .data[[facet_by]], role) %>%
  arrange(role, facet_val) %>%
  pull(facet_val) %>%
  as.character()

touch_area_data_plot <- touch_area_data %>%
  mutate(
    role = factor(role, levels = role_levels),
    !!rlang::sym(facet_by) := factor(.data[[facet_by]], levels = facet_order)
  )

touch_area_data_plot <- touch_area_data_plot %>%
  ...
  player_touch_map <- ggplot(touch_area_data) +
    ...
    facet_wrap(factor(role, levels = role_levels) ~ playerId) +
    labs(
      title = teamtitle,
      x = "",
...
#set away
Match_Away <- paste0("Man Utd")

#Set Home
Match_Home <- paste0("Liverpool")

#set match date
Game_date <- paste("19-10-2025")
